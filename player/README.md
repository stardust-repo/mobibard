...
`v4.5`에서는 피아노롤 시간 격자를 고정 픽셀 간격에서 실제 MML `L4` 1박 간격으로 변경했습니다. 격자는 피아노 건반 윗선에서 시작해 재생 시간과 템포 변화에 맞춰 위에서 아래로 흐르며, 각 템포 변경 지점은 얇은 `T숫자` 가로선으로 표시됩니다. 진행 슬라이더의 `T숫자` 라벨이나 피아노롤 왼쪽 `T숫자` 버튼을 선택하면 해당 템포를 `32~255` 범위에서 바로 수정할 수 있습니다. 피아노롤 가로 템포선은 표시 전용이며, 왼쪽 버튼은 마우스를 올리면 외곽선·크기·그림자가 변합니다. 편집 도구에는 선택 채널의 기존 `O숫자` 명령만 일괄 증감하는 `음정 조절`도 추가했습니다. `MIDI 추출`, `강약 생성`, `장르 편곡`에는 공통 `.beta-feature` 스타일이 그리는 노란색 원형 B 마커를 표시하며, 전체 MML 탭의 글자 수는 총합과 채널별 색상 숫자를 두 줄로 보여 줍니다. 장르 편곡에는 `팝`과 `클래식`을 추가하고 기본 장르를 팝으로 변경했습니다. `강약 생성`은 노트의 길이·밀도·쉼표·음정 흐름을 분석해 선택한 장르와 변형 강도에 맞는 `V` 변화를 자동 생성합니다.

`v4.3`에서는 MML 편집 도구에 시범 기능인 `장르 편곡`을 추가했습니다. 선택하거나 자동 감지한 멜로디를 유지한 채 화음1~5를 `팝` / `재즈` / `발라드` / `보사노바` / `록` / `펑크` / `클래식` 스타일 반주로 다시 만듭니다. 변형 강도는 `가볍게` / `보통` / `강하게` 중에서 선택할 수 있습니다.
...
└─ js/
   ...
   ├─ mml-optimizer.js         # 자동 최적화, 장르 편곡, 강약 생성, 쉼표 삭제, 볼륨 조절, O숫자 음정 조절, 시작 공백, 나눠복사
...
| `js/mml-optimizer.js` | `window.MabiOptimizer` | `optimizeMml`, `optimizePart`, `arrangeGenreMml`, `generateDynamicsMml`, `trimShortRestsMml`, `adjustVolumesMml`, `transposeOctavesMml`, `addLeadingSilenceMml`, `splitMmlPages` |
...
- `시작 공백`, `쉼표 삭제`, `볼륨 조절`, `음정 조절`, `강약 생성`, `장르 편곡` 편집 기능을 제공합니다.
...
| `dynamicsGenerateDialog` | 강약 장르, 변형 강도, 적용 채널 선택 |
| `tempoEditDialog` | 슬라이더/피아노롤에서 선택한 템포를 `32~255` 범위로 수정 |
| `leadingSilenceDialog` | 시작 공백 초 단위 입력 |
| `genreArrangeDialog` | 팝/재즈/발라드/보사노바/록/펑크/클래식 장르, 변형 강도, 멜로디 기준 채널 선택 |
...
- `강약 생성` 버튼에서 BETA Dialog를 엽니다.
- 강약 장르는 `팝`, `재즈`, `발라드`, `보사노바`, `록`, `펑크`, `클래식` 중에서 선택합니다. 기본값은 `팝`입니다.
...
- 사용 중인 `V` 값이 3종 이상이거나 최대·최소 차이가 2 이상인 채널은 기존 강약이 감지된 것으로 판단합니다.
...
| `rest_trim_apply`, `bulk_volume_adjust`, `bulk_pitch_adjust`, `dynamics_generate_apply`, `leading_silence_apply`, `genre_arrange_apply` | 편집 도구 적용 성공 | 도구별 설정값과 적용 채널 수 |
...
- [ ] `강약 생성` 버튼이 음정 조절 오른쪽에 표시되고 공통 원형 B 마커가 붙는지
- [ ] `강약 생성`에서 팝/재즈/발라드/보사노바/록/펑크/클래식과 가볍게/보통/강하게를 선택할 수 있는지
- [ ] 조건 안의 채널은 즉시 생성되고, 기존 강약 조건을 벗어난 채널은 판단 근거 확인 후 `그래도 생성`을 눌렀을 때만 교체되는지
- [ ] 강약 생성 결과의 소리 있는 음표가 `V2~V15` 범위를 벗어나지 않고 원본 음표와 길이가 유지되는지
- [ ] 장르 편곡의 기본 선택이 팝이며 팝과 클래식이 서로 다른 반주 패턴을 만드는지
...
- 장르 편곡에 `팝`과 `클래식`을 추가하고 기본 선택 장르를 팝으로 변경했습니다.
- 팝 편곡은 안정적인 3화음, 규칙적인 코드 펄스, 루트/5도 베이스를 사용합니다.
- 클래식 편곡은 성부 진행, 분산화음과 알베르티형 저음 진행을 사용합니다.
- `음정 조절` 오른쪽에 BETA 기능 `강약 생성` 버튼과 Dialog를 추가했습니다.
- 강약 생성은 장르와 변형 강도를 기준으로 노트 길이·밀도·쉼표·음정 흐름을 분석해 `V` 변화를 생성합니다.
- 강약 생성 시 기존 강약이 감지되면 `V 값 3종 이상` 또는 `최대·최소 차이 2 이상`의 실제 판단 근거와 채널을 보여 주고, 확인 후 덮어쓸 수 있도록 변경했습니다.
- 기존 강약 판단 조건을 벗어난 채널은 채널별 근거를 확인한 뒤 사용자가 `그래도 생성`을 선택한 경우에만 기존 `V` 변화를 교체합니다.
...
- BETA 마커를 버튼 내부 문자 대신 공통 `.beta-feature::after` 아이콘으로 렌더링해 버튼명이 `강약 생성B`처럼 붙어 보이는 문제를 수정했습니다.
