...
상단 저장 영역 오른쪽의 `MIDI 추출` 버튼에서 MuScriptor 로컬 실행 패키지를 내려받을 수 있습니다. 그 오른쪽의 `리듬게임` 버튼은 전체 화면 팝업 레이어에서 이웃 경로 `../mobibeats/`의 모비비트를 열고, 현재 MML과 6개 채널 악기 정보를 전달합니다. `muscriptor_win.zip`과 `muscriptor_mac.zip`은 `assets/midi-extract/packages/`에 포함됩니다.
