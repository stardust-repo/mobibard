...
사용자가 MIDI 또는 MusicXML 파일을 불러오면 `midiConvertDialog`가 열립니다. 이 Dialog는 “입력 파일 안의 악기들을 마비노기 MML의 1~6개 채널에 어떻게 배치할지” 정하는 화면입니다. 상단 안내는 별도 상자 없이 기능별 한 줄 설명으로 표시합니다. `MIDI/MusicXML 듣기`, MML 채널, 역할, 겹침 병합, 미리 듣기/변환의 의미를 바로 확인할 수 있습니다.
...
4. 각 MML 채널마다 역할, 겹침 병합 방식, 사용할 악기를 직접 조정하고, 상단 토글에서 64박 또는 32박 양자화를 선택할 수 있습니다.
...
- 화면에는 `악기명 · 파트명` 순서로 선택지를 표시합니다.
- 사용자 메뉴는 `파트(채널 표시명) + Program 번호`가 같은 항목을 하나로 묶습니다.
- Bank MSB/LSB가 달라도 같은 파트와 Program이면 같은 메뉴로 묶으며, Bank 번호는 메뉴 판단이나 표시에 사용하지 않습니다.
- 내부 변환과 원본 미리듣기를 위해 원본 MIDI 채널/Bank/Program 정보는 노트에 계속 보존합니다.
...
- [ ] MIDI/MusicXML 변환 Dialog 상단 안내가 별도 상자 없이 기능별 한 줄로 표시되는지
...
- MIDI/MusicXML 변환 Dialog의 중첩 설명 상자와 접기/펼치기를 제거하고, 원본 듣기·MML 채널·역할·겹침 병합·미리 듣기/변환을 한 줄씩 표시하도록 정리했습니다.
...
- MIDI에 비트 악기가 있으면 초록색으로 `N개의 비트 악기가 있습니다.`라고 표시하고, 없으면 `비트 악기가 없습니다.`라고 표시합니다.
