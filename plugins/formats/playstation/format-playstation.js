(() => {
  ...
  function lightweightDetect(bytes) {
    const view = core.asUint8Array(bytes);
    if (asciiAt(view, 0, "SCEIVers") && asciiAt(view, 0x10, "SCEISequ") && asciiAt(view, 0x30, "SCEIMidi")) return true;
    return asciiAt(view, 0, "pQES");
  }
  ...
  core.registerFormat({
    ...
    detect(bytes) {
      return root.MabiPlayStationSequence?.detect ? Boolean(root.MabiPlayStationSequence.detect(bytes)) : lightweightDetect(bytes);
    },
