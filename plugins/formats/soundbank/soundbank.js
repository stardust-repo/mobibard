(() => {
  ...
  const FALLBACK_MESSAGES = Object.freeze({
    ...
    "sound.err_decode_unavailable": "이 브라우저에서는 SF3 압축 샘플을 해제할 수 없습니다.",
    "sound.err_sf3_codec": "SF3 샘플을 해석하지 못했습니다: {0}",
    "sound.err_sf3_mono": "SF3 샘플의 채널 구성을 지원하지 않습니다: {0}",
    "sf2.err_header": "올바른 SF2/SF3 파일이 아닙니다.",
    "sf2.err_sections": "SF2/SF3 내부 섹션이 없습니다.",
    "sf2.err_samples": "SF2/SF3 샘플 데이터가 없습니다.",
    "sf2.err_headers": "SF2/SF3 프리셋 또는 샘플 헤더가 없습니다.",
    "sf2.err_table": "SF2/SF3 테이블을 읽지 못했습니다. 레코드 크기: {0}",
    "dls.err_header": "올바른 DLS 파일이 아닙니다.",
    "dls.err_sections": "DLS 내부 섹션이 없습니다.",
    "dls.err_waves": "DLS 웨이브 데이터가 없습니다.",
    "dls.err_instruments": "DLS 악기 프리셋이 없습니다.",
  });
  const tr = (key, values = []) => {
  ...
  let embeddedSoundBank = null;
  let embeddedSoundBankPromise = null;
  ...
  async function parseSoundBank(bytes, options = {}) {
    const source = utils.toUint8Array(bytes);
    const candidates = utils.macBinaryForkCandidates(source);
    let firstError = null;
    for (const candidate of candidates) {
      const data = candidate.bytes;
      if (data.length < 12 || ascii(data, 0, 4) !== "RIFF") continue;
      const type = ascii(data, 8, 4);
      try {
        let bank = null;
        if (type === "sfbk") bank = await parseSoundFont(data, options);
        else if (type === "DLS ") bank = await parseDls(data, options);
        else continue;
        bank.container = { ...candidate.metadata };
        return bank;
      } catch (error) {
  ...
  async function parseSoundBankFile(file, options = {}) {
    if (!file || typeof file.arrayBuffer !== "function") throw new TypeError("SoundBank File 객체가 필요합니다.");
    const bank = await parseSoundBank(await file.arrayBuffer(), options);
    bank.fileName = String(bank.container?.fileName || file.name || "soundbank");
    bank.originalFileName = String(file.name || "");
    return bank;
  }

  async function decodeBase64(base64, options = {}) {
    const encoded = String(base64 || "").replace(/\s+/g, "");
    if (!encoded) throw new Error("내장 SoundBank 데이터가 비어 있습니다.");
    const binary = atob(encoded);
    const bytes = new Uint8Array(binary.length);
    const chunkSize = Math.max(64 * 1024, Number(options.chunkSize) || 256 * 1024);
    for (let offset = 0; offset < binary.length; offset += chunkSize) {
      const end = Math.min(binary.length, offset + chunkSize);
      for (let index = offset; index < end; index++) bytes[index] = binary.charCodeAt(index) & 0xff;
      options.onProgress?.(binary.length ? end / binary.length : 1);
      if (options.yield !== false && end < binary.length) await new Promise(resolve => setTimeout(resolve, 0));
    }
    return bytes;
  }

  async function loadEmbeddedSoundBank(options = {}) {
    if (embeddedSoundBank && options.reload !== true) return embeddedSoundBank;
    if (embeddedSoundBankPromise && options.reload !== true) return embeddedSoundBankPromise;
    const encoded = options.base64 != null
      ? String(options.base64 || "")
      : String(window.MOBIBARD_DEFAULT_SF3_BASE64 || "");
    if (!encoded.trim()) throw new Error("기본 SoundBank 데이터를 찾지 못했습니다.");
    const name = String(options.name || window.MOBIBARD_DEFAULT_SF3_NAME || "default.sf3");
    embeddedSoundBankPromise = (async () => {
      const bytes = await decodeBase64(encoded, {
        onProgress: options.onProgress,
        yield: options.yield,
        chunkSize: options.chunkSize,
      });
      const bank = await parseSoundBank(bytes, options);
      bank.fileName = name;
      bank.isEmbeddedDefault = true;
      embeddedSoundBank = bank;
      if (options.clearBase64 !== false && options.base64 == null) {
        try { window.MOBIBARD_DEFAULT_SF3_BASE64 = ""; } catch (_) {}
      }
      return bank;
    })().catch(error => {
      embeddedSoundBankPromise = null;
      throw error;
    });
    return embeddedSoundBankPromise;
  }

  function findPreset(soundBank, program = 0, bankNumber = 0) {
  ...
  function resetEmbeddedSoundBankCache() {
    embeddedSoundBank = null;
    embeddedSoundBankPromise = null;
  }
  ...
  function isSupportedFile(fileName) {
    return [...SUPPORTED_EXTENSIONS, "bin", "macbin"].includes(extensionOf(fileName));
  }

  function selectRiffContainer(value, expectedType = "") {
    for (const candidate of utils.macBinaryForkCandidates(value)) {
      const data = utils.toUint8Array(candidate.bytes);
      if (data.length < 12 || ascii(data, 0, 4) !== "RIFF") continue;
      const type = ascii(data, 8, 4);
      if (!expectedType || type === expectedType) {
        return { bytes: data, type, metadata: { ...candidate.metadata } };
      }
    }
    return null;
  }
  ...
  function detectFormat(bytes) {
    const selected = selectRiffContainer(bytes);
    if (selected?.type === "DLS ") return "dls";
    if (selected?.type === "sfbk") return "soundfont";
    return "";
  }

  async function parseSoundFont(value, options = {}) {
    const selected = selectRiffContainer(value, "sfbk");
    if (!selected) throw new Error(tr("sf2.err_header"));
    ...
    buildSfPresets(sf);
    sf.container = { ...selected.metadata };
    return sf;
  }
  ...
  async function parseDls(value, options = {}) {
    const selected = selectRiffContainer(value, "DLS ");
    if (!selected) throw new Error(tr("dls.err_header"));
    const bytes = selected.bytes;
    const root = parseRiff(bytes, 12, bytes.length);
    ...
    bank.container = { ...selected.metadata };
    return bank;
  }
