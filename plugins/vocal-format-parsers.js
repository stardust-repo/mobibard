(() => {
  ...
  function parseVsq(bytes, fileName) {
    ...
    const chunkId = offset => String.fromCharCode(...view.subarray(offset, offset + 4));
    if (view.length >= 4 && chunkId(0) === "MThd") {
      return { midiBytes: view, metadata: { midiBased: true, macBinary: false } };
    }
    if (view.length >= 132 && chunkId(128) === "MThd") {
      const dataForkLength = ((view[83] << 24) | (view[84] << 16) | (view[85] << 8) | view[86]) >>> 0;
      const end = 128 + dataForkLength;
      if (dataForkLength >= 14 && end <= view.length) {
        return { midiBytes: view.slice(128, end), metadata: { midiBased: true, macBinary: true } };
      }
    }
