...
function parseGp5(bytes) {
  ...
  const readChord=(strings,beat)=>{skip(17);beat.chordName=strByte(21);skip(4);i32();for(let i=0;i<7;i++)i32();skip(32);};
  ...
  const readMixChange=tempo=>{ i8();skip(16);const vals=[i8(),i8(),i8(),i8(),i8(),i8()];strByteIntSize();const tv=i32();for(const v of vals)if(v>=0)i8();if(tv>=0){tempo.value=tv;skip(1);if(versionIndex>0)skip(1);}i8();skip(1);if(versionIndex>0){strByteIntSize();strByteIntSize();}};
  const readBeat=(start,measure,track,tempo,voiceIndex)=>{const flags=u8();const beat=getBeat(measure,start),voice=beat.voices[voiceIndex];if(flags&64){const type=u8();voice.empty=(type&2)===0;}const dur=durationTicks(flags);if(flags&2)readChord(track.strings,beat);if(flags&4)beat.text=strByteIntSize();const effect={};if(flags&8)readBeatEffects(beat,effect);if(flags&16)readMixChange(tempo);const stringFlags=u8();for(let i=6;i>=0;i--){if((stringFlags&(1<<i))!==0 && (6-i)<track.strings.length){const s={...track.strings[6-i]};voice.notes.push(readNote(s,track,{...effect}));}}voice.duration=dur;skip(1);const read=i8();if(read&2)skip(1);return voice.notes.length?dur:0;};
