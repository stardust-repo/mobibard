...
3. 건반 범위(시작~끝)를 지정한 뒤, 건반이 눌리지 않은 프레임에서 건반을 검출해 기준색을 확정합니다.
4. 2번 `영상 분석`에서 재생 버튼 바로 아래의 한 줄 분석 시간을 확인·조정하고, 건반 인식 설정을 마친 뒤 섹션 맨 아래의 `영상 분석`을 실행합니다. 첫 건반 검출 때 설정한 건반 범위가 안정적으로 보이는 시작/종료 시간을 한 번 자동 탐색하며 `자동 탐색`으로 다시 찾을 수 있습니다.
...
- **1선 모드**: 흰/검 건반이 같은 평면의 스트립으로 그려진 영상용입니다. 선택한 시작~끝 음계의 건반 수만큼 연속 검출하고 같은 선에서 모든 건반의 색상을 샘플링합니다.

- In 1-line mode, if key boundaries cannot be found reliably, the selected guide span is divided into the number of keys in the configured start/end pitch range and the UI marks the result as estimated. Vertical orientation now starts with the guide spanning the full video height.
...
- `건반 범위`의 `자동 탐색`은 현재 프레임에서 흰 건반 수와 검은 건반 배열을 분석해 시작~끝 건반 범위를 제안합니다.
- 전체 52개 흰 건반이 검출되면 A0~C8(88건반)로 확정합니다. 부분 건반은 검은 건반 패턴으로 시작 음 이름을 추정하고, 절대 옥타브를 영상만으로 확정할 수 없는 경우 현재 설정 중심에 가장 가까운 옥타브 후보를 선택합니다.
- 자동 탐색이 실패하거나 옥타브가 다른 경우 `코드 시작 ~ 코드 끝` 선택값을 직접 조정할 수 있습니다.

## 분석 시간 자동 탐색

- `건반 검출`이 완료된 뒤에만 `자동 탐색` 버튼이 활성화됩니다.
- 영상 앞쪽에서는 설정한 시작~끝 건반 범위가 안정적으로 확인되는 첫 프레임을, 뒤쪽에서는 마지막 프레임을 찾아 분석 시작/종료 시간에 설정합니다.
- 2선 모드는 설정한 음계 범위에 포함되는 흰 건반 수와 MIDI 매핑을 확인하고, 1선 모드는 설정한 전체 건반 수의 실제 경계 또는 흰/검 스트립 대비를 함께 확인합니다. 단순 균등 분할 fallback만으로 영상 전체를 건반 구간으로 오인하지 않습니다.
...
- 분석 시간 자동 탐색은 설정한 건반 범위의 존재 여부뿐 아니라 건반 검출 기준 프레임과의 OKLab 외형 유사도를 함께 사용하며, 페이드 인/아웃 구간에서는 더 안정적인 내부 지점으로 보정합니다.

## 건반 범위

- 영상에 보이는 실제 건반의 시작 음과 끝 음을 A0~C8 범위에서 지정할 수 있습니다. 기본값은 88건반 피아노의 A0~C8입니다.
- 건반 범위를 변경하면 기존 건반 검출과 분석 결과를 무효화하고 다시 `건반 검출`하도록 합니다. 새 범위에서 첫 검출이 성공하면 분석 시간 자동 탐색도 다시 한 번 실행됩니다.
- `자동 탐색`은 고정 88건반을 찾는 것이 아니라 현재 지정된 건반 범위가 안정적으로 보이는 구간을 찾습니다.
- 1선 모드의 균등 영역 수도 지정한 시작~끝 건반 수에 맞춰집니다.
