export default {
  code: 'ko', htmlLang: 'ko', strings: {
    ...
    'tutorial.step2_part': '건반 설정', 'tutorial.step2_title': '인식선 맞추기', 'tutorial.step2_body': '2선 모드 또는 1선 모드를 선택하고 수평 건반 또는 수직 건반을 선택합니다. 2선은 흰/검 건반에 각각, 1선은 88건반 전체를 가로지르도록 인식선을 맞춘 뒤 변화량을 조절합니다.', 'tutorial.step2_note': '흰 건반 변화량은 기본 20%, 검은 건반 변화량은 기본 30%입니다. 값이 클수록 더 큰 색 변화만 눌림으로 인식합니다.',
    'tutorial.step3_part': '건반 검출', 'tutorial.step3_title': '기준색 확정하기', 'tutorial.step3_body': '건반이 눌리지 않은 프레임에서 「건반 검출」을 누릅니다. 작은 박스와 기준색이 표시되고, 이후 현재 프레임에서 변화가 감지된 샘플 박스가 바로 밝게 표시됩니다.', 'tutorial.step3_note': '새 영상을 연 뒤 첫 건반 검출에서는 88건반이 보이는 분석 시작·종료 시간을 한 번 자동 탐색합니다. 이후에는 분석 시간의 「자동 탐색」으로 다시 찾을 수 있습니다. 인식선을 다시 움직이면 재검출이 필요합니다.',
    'tutorial.step4_part': '분석과 MIDI', 'tutorial.step4_title': '영상 분석 후 MIDI 조정', 'tutorial.step4_body': '분석 시간을 정해 영상 분석을 실행합니다. 분석이 끝난 뒤 3번 MIDI 다운로드에서 노트 길이 확장, 템포, 벨로시티를 바꾸면 영상을 다시 분석하지 않고 MIDI가 즉시 다시 계산됩니다.', 'tutorial.step4_note': '노트 길이 확장은 0 / 0.25 / 0.5 / 1 프레임 중 하나를 선택합니다. 오디오 볼륨 반영도 분석된 노트의 시작 위치를 기준으로 적용할 수 있습니다.',
