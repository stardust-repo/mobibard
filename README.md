...
- 공용 MML 최적화기에 템포 정리를 추가했습니다. 사용자 안내는 `1마디 안에서 편차가 10 미만인 템포들을 삭제합니다.`로 단순화했습니다. Simple은 선행 무음 추가 전에 이를 기본 적용하고, Player는 삭제 예정 템포 수를 확인한 뒤 적용합니다.
...
| Player | 화면 제목의 버전 글씨를 절반 크기로 조정하고, Guest 계정 버튼을 모든 상태에서 38×38 정사각형으로 고정했습니다. Guest 상태에서는 투명 사람 아이콘이 흰색 버튼 배경 위에 표시되며, 테마 변경 문구 앞에는 Simple과 같은 `◐` 표식을 사용합니다. |
