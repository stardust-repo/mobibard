...
- 공개 다운로드는 `assets/MobiBard_Instruments_sf3.zip`과 `assets/MobiBard_Instruments_dls.zip` 두 압축 파일로 제공합니다. SF3 ZIP 안의 `MobiBard_Instruments.sf3`는 `assets/default_sf3.js`에 내장된 기본 음색과 동일한 파일이며, DLS ZIP에는 정리된 호환용 `MobiBard_Instruments.dls`가 들어 있습니다. 프로젝트에는 다운로드용 원본 SF3/DLS를 중복 보관하지 않습니다. 두 압축 파일과 그 안의 음색 파일은 개인·상업적 이용, 복사, 수정, 변환, 재배포, 미러링 및 웹에서의 직접 링크를 제한 없이 허용하며 출처 표기나 별도 허가가 필요하지 않습니다. Player와 Editor의 사운드폰트 설정 팝업에서도 SF3/DLS ZIP을 바로 다운로드할 수 있습니다. 자세한 자유 이용 안내는 `assets/MobiBard_Instruments_LICENSE.txt`를 참고합니다.
